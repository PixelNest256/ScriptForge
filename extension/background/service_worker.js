...
function isEvalCall(node) {
  ...
  if (node.callee.type === "MemberExpression" && !node.callee.computed && node.callee.property?.name === "eval") {
    return true;
  }
  return false;
}
...
function isExternalScriptSrc(node) {
  ...
  if (obj?.type === "Identifier" && obj.name === "script" && node.right?.type === "Literal" && typeof node.right.value === "string" && /^https?:\/\//.test(node.right.value)) {
    return true;
  }
  if (obj?.type === "MemberExpression" && obj.property?.name === "src" && node.right?.type === "Literal" && typeof node.right.value === "string" && /^https?:\/\//.test(node.right.value)) {
    return true;
  }
  return false;
}

// src/analyzer/permission_detector.js
...
function detectPermissions(ast) {
  ...
  const networkDomains = /* @__PURE__ */ new Set();
  simple(ast, {
    CallExpression(node) {
  ...
  return Array.from(found.values()).map((p) => ({
    ...
    ...p,
    domains: p.domains?.length ? [...p.domains] : void 0
  }));
}
function addPerm(found, id, extra = {}) {
  const existing = found.get(id) || { id, domains: [] };
  if (extra.domains) {
    existing.domains = [.../* @__PURE__ */ new Set([...existing.domains || [], ...extra.domains])];
  }
...
function checkNetwork(node, found, domains) {
  ...
  if (callee.type === "MemberExpression" && callee.object?.name === "XMLHttpRequest" && callee.property?.name === "open") {
...
function isAddEventListener(node, events) {
  const c = node.callee;
  if (c.type === "MemberExpression" && c.property?.name === "addEventListener" && node.arguments[0]?.type === "Literal") {
    return events.includes(node.arguments[0].value);
  }
  return false;
}
function checkCookie(node, found) {
  if (node.object?.type === "Identifier" && node.object.name === "document" && node.property?.name === "cookie") {
...
function checkDomWrite(node, found) {
  const c = node.callee;
  if (c.type === "MemberExpression" && ["appendChild", "insertBefore", "replaceChild", "removeChild"].includes(c.property?.name)) {
...
async function sha256(text) {
  ...
  const data2 = new TextEncoder().encode(normalized);
...
async function verifyScriptHash(script) {
...
function getMatchPatterns(script) {
  let patterns = script.matches?.length ? [...script.matches] : parseUserscriptMeta(script.code).meta.match || [];
  patterns = normalizeMatchPatterns(patterns.map((p) => p.trim()).filter(Boolean));
  return patterns.length ? patterns : ["<all_urls>"];
}
function wrapUserScriptBody(body) {
...
async function getMatchingScriptsForUrl(url) {
  const scripts = await getScripts();
  const result = [];
...
var SCRIPTFORGE_REQUEST = "scriptforge-request";
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === SCRIPTFORGE_REQUEST && sender.tab) {
    getMatchingScriptsForUrl(message.url).then((scripts) => {
